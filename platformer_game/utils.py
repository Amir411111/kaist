...
def create_character_sprite(color, character_id="1"):
    ...
    """
    # Маппинг ID персонажей к файлам (используем женские персонажи)
    character_files = {
    ...
    surface.fill(PURPLE)  # Фиолетовый для женских персонажей
    pygame.draw.rect(surface, BLACK, (0, 0, PLAYER_WIDTH, PLAYER_HEIGHT), 4)
    
    # Добавляем крупный текст
    font = pygame.font.Font(None, 36)
    text_surface = font.render(f"F{character_id}", True, WHITE)
    text_rect = text_surface.get_rect(center=(PLAYER_WIDTH//2, PLAYER_HEIGHT//2))
    surface.blit(text_surface, text_rect)
    
    return surface

def create_male_character_sprite(character_id="1"):
    """
    Создает спрайт мужского персонажа для пленных
    """
    # Маппинг ID персонажей к файлам (мужские персонажи)
    character_files = {
        "1": "assets/characters/male_1.png",
        "2": "assets/characters/male_2.png", 
        "3": "assets/characters/male_3.png"
    }
    ...
    if character_id in character_files:
        ...
        if os.path.exists(character_path):
            try:
                ...
                print(f"DEBUG: Error loading male character image: {e}")
    
    # Если не удалось загрузить - создаем заглушку
    surface = pygame.Surface((PLAYER_WIDTH, PLAYER_HEIGHT))
    surface.fill(BLUE)  # Синий для мужских персонажей
    pygame.draw.rect(surface, BLACK, (0, 0, PLAYER_WIDTH, PLAYER_HEIGHT), 4)
    
    # Добавляем крупный текст
    font = pygame.font.Font(None, 36)
    text_surface = font.render(f"M{character_id}", True, WHITE)
    text_rect = text_surface.get_rect(center=(PLAYER_WIDTH//2, PLAYER_HEIGHT//2))
    surface.blit(text_surface, text_rect)
...
def create_enemy_sprite():
    """
    Создает спрайт врага, используя уменьшенную версию босса
    """
    boss_path = "assets/boss/hippo_boss.png"
    
    # Пытаемся загрузить изображение босса и уменьшить его для врагов
    if os.path.exists(boss_path):
        try:
            image = pygame.image.load(boss_path)
            # Масштабируем изображение под размер врага (уменьшенная версия босса)
            image = pygame.transform.scale(image, (ENEMY_WIDTH, ENEMY_HEIGHT))
            return image
        except Exception as e:
            print(f"DEBUG: Error loading boss image for enemy: {e}")
...
def create_boss_sprite():
    ...
    # Пытаемся загрузить изображение босса-бегемота
    if os.path.exists(boss_path):
        try:
            image = pygame.image.load(boss_path)
            # Масштабируем изображение под размер босса
